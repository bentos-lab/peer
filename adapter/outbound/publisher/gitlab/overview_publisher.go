...
// OverviewCommentClient posts overview comments to GitLab merge requests.
...
// OverviewPublisher publishes overview comments to GitLab merge requests.
type OverviewPublisher struct {
	client OverviewCommentClient
	logger usecase.Logger
}

// NewOverviewPublisher creates a GitLab overview publisher.
...
// PublishOverview posts a markdown overview comment to one GitLab merge request.
func (p *OverviewPublisher) PublishOverview(ctx context.Context, req usecase.OverviewPublishRequest) error {
	if !shouldPublishOverviewForAction(req.Metadata["action"]) {
		p.logger.Infof(
			"Skipped GitLab overview comment for %q!%d action=%q.",
...
func (p *OverviewPublisher) logOverviewPayload(state string, req usecase.OverviewPublishRequest, body string) {
	p.logger.Debugf("GitLab overview comment metadata state=%q repo=%q mr=%d action=%q categories=%d walkthroughs=%d.",
	...
	p.logger.Tracef("GitLab overview comment content state=%q body=%q.", state, body)
}
