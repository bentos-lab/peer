...
Purpose: Run a repository review for a change request or local diff.
...
- `--publish` (default: `false`) post review result as pull request comments. Requires `--change-request`.
- `--suggest` (default: unset; falls back to recipe/config, config default is `false`) enable suggested code changes in review findings.
...
Review PR 123 and publish comments:
```bash
peer review --change-request 123 --publish
```

Review using explicit VCS provider:
```bash
peer review --vcs-provider github --change-request 123
```

Review a specific repo by URL:
```bash
peer review --repo https://github.com/example/repo.git --change-request 123
```

Review local workspace diff (defaults to base `HEAD`, head `@all`):
```bash
peer review
```

Review local staged diff only:
```bash
peer review --head @staged
```

Review local diff between two refs:
```bash
peer review --base main --head feature-branch
```

Enable suggested changes:
```bash
peer review --change-request 123 --suggest
```

Use OpenAI shortcut with default model:
```bash
peer review --change-request 123 --llm-openai-base-url openai --llm-openai-api-key "$OPENAI_API_KEY"
```
...
Use Gemini shortcut with explicit model:
```bash
peer review --change-request 123 --llm-openai-base-url gemini --llm-openai-model gemini-2.5-pro
```

Use coding-agent overrides:
```bash
peer review --change-request 123 --code-agent opencode --code-agent-provider opencode --code-agent-model "model-x"
```

Increase verbosity:
```bash
peer review --change-request 123 -v
peer review --change-request 123 -vv
```
